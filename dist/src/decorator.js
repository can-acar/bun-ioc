...
class LazyLock2 {
  ...
  getSync() {
    ...
    const v = this.resolver();
    if (v instanceof Promise)
      throw new Error("Sync path hit async factory");
    this.value = v;
    this.resolved = true;
    return v;
  }
...
function Service(opts = {}) {
  ...
  return function(target) {
    const t = tokenOverride ?? token(target.name);
    globalContainer.register(t, (c) => {
      const paramTypes = Reflect.getMetadata("design:paramtypes", target) || [];
      const names = Reflect.getMetadata("di:paramNames", target) || [];
      const params = paramTypes.map((p, i) => {
        const pt = token(p.name);
        const nm = names[i];
...
function Inject(tokenOrClass, name) {
  return function(target, propertyKey) {
    ...
    Object.defineProperty(target, propertyKey, {
      get() {
        lock ??= new LazyLock2(() => globalContainer.resolve(injectToken, propName));
        return lock.getSync();
      },
      enumerable: true,
      configurable: true
...
function InjectLazy(tokenOrClass, name) {
  return function(target, propertyKey) {
    const injectToken = resolveToken(tokenOrClass, target, propertyKey);
    const propName = name ?? Reflect.getMetadata("di:propName", target, propertyKey);
    let lock;
    Object.defineProperty(target, propertyKey, {
      get() {
        lock ??= new LazyLock2(() => globalContainer.resolveAsync(injectToken, propName));
        return lock.get();
      },
      enumerable: true,
      configurable: true
...
function InjectMethod() {
  return function(target, propertyKey, descriptor) {
    ...
    descriptor.value = function(...args) {
      ...
      const names = Reflect.getMetadata(`di:paramNames:${propertyKey}`, target) || [];
      const resolved = types.map((t, i) => args[i] ?? globalContainer.resolve(token(t.name), names[i]));
...
function resolveToken(tokenOrClass, target, propertyKey) {
  if (typeof tokenOrClass === "symbol")
    return tokenOrClass;
  if (typeof tokenOrClass === "function")
    return tokenOrClass["di:token"] || token(tokenOrClass.name);
  const type = Reflect.getMetadata("design:type", target, propertyKey);
  return token(type?.name ?? String(propertyKey));
}
export {
  globalContainer,
  Service,
